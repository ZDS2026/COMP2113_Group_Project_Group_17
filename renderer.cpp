...
void draw_frame(
    ...
    for (int r = 0; r < rows; ++r) {
        ...
        if (r == 0) {
            ...
            // Single bottom frame row (same style as top). Do not add another grey_frame
            // below it — that looked like a second grey bar when HUD is taller than map.
            line << map_top;
        } else {
            line << std::string(map_frame_width, ' ');
        }
        line << "   ";
