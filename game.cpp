...
void Game::show_help_screen() const {
    ...
    std::cout << "  Each monster has 40% chance to not move on this turn.\n";
    ...
    std::cout << "  Monsters are blocked by walls, other monsters, and items (H/S/D).\n\n";
...
bool Game::cell_blocked_for_monster(int r, int c, int self_idx) const {
    ...
    // Potion, Sword, and Shield all block monsters
...
void Game::tick_monsters() {
    ...
    for (int i = 0; i < monster_count && running; ++i) {
        ...
        // Monsters have probability not to move this turn (except Boss which never moves)
        if ((std::rand() % 100) < 40) continue;  // 40% chance to not move
