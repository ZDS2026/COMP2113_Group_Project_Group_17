...
**Multi-floor run:** Easy has **3** floors, Medium **4**, Hard **5**. Clearing a floor means beating that floor’s Boss; you then enter the **next** floor with a **new map** while keeping your stats. **Win** the run by beating the Boss on the **last** floor. Deeper floors have tougher enemies (especially regular monsters).
...
| | Name | UID |
|--|------|-----|
| A (Leader) | `<Yu Zhenkai>` | `<3036483313>` |
...
- **Multi-floor progression** (3 / 4 / 5 floors by difficulty); boss on each floor unlocks the next; saves store current floor  
